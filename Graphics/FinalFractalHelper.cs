...
namespace Hsmod.Graphics
{
        ...
        private static Dictionary<int, FinalFractalHelper.FinalFractalProfile> _fractalProfiles = new Dictionary<int, FinalFractalHelper.FinalFractalProfile>()
    {
      ...
      },
      {
        ...
        {
            Color color = Color.Lerp(Color.White, Color.Violet, Utils.Utils.GetLerpValue(0.0f, 0.7f, progressOnStrip, true)) * (1f - Utils.Utils.GetLerpValue(0.0f, 0.98f, progressOnStrip, false));
            color.A /= (byte)2;
            return color;
        }
        ...
        public struct FinalFractalProfile
        {
            ...
            private void StripDust(Vector2 centerPosition, float rotation, Vector2 velocity)
            {
                ...
                {
                    Dust dust = Dust.NewDustPerfect(centerPosition, 278, new Vector2?(), 100, Color.Lerp(this.trailColor, Color.White, Main.rand.NextFloat() * 0.3f), 1f);
                    dust.scale = 0.4f;
            ...
            private Color StripColors(float progressOnStrip)
            {
                Color color = this.trailColor * (1f - Utils.Utils.GetLerpValue(0.0f, 0.98f, progressOnStrip, false));
                color.A /= (byte)2;
                return color;
            }
