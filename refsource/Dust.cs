...
namespace Terraria
{
	public class Dust
	{
		...
		{
			Dust dust = Main.dust[Dust.NewDust(pos, 0, 0, 6, 0f, 0f, 0, default(Color), 1f)];
			dust.position = pos;
			dust.velocity = Vector2.Zero;
			dust.fadeIn = 1f;
			dust.noLight = true;
		...
		public static void UpdateDust()
		{
			...
			int num114 = num;
			if ((double)num114 > (double)Main.numDust * 0.9)
			{
				Dust.dCount = 0.9f;
				return;
			}
			if ((double)num114 > (double)Main.numDust * 0.8)
			{
				Dust.dCount = 0.8f;
				return;
			}
			if ((double)num114 > (double)Main.numDust * 0.7)
			{
				Dust.dCount = 0.7f;
				return;
			}
			if ((double)num114 > (double)Main.numDust * 0.6)
			{
				Dust.dCount = 0.6f;
				return;
			}
			if ((double)num114 > (double)Main.numDust * 0.5)
			{
				Dust.dCount = 0.5f;
				return;
			}
